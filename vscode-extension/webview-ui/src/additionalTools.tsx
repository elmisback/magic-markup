  ...
  const generateExplanation = async () => {
    ...
    **Why it's needed:**
    ...
    Keep each bullet point to 1-2 sentences maximum. This should be easy to read without scrolling.`,
          },
          {
            role: "user",
            content:
              `Here is the code with the highlighted section:\n\n${
    ...
    **Why it's needed:**
    ...
    Each value should be a single string with 3-5 sentences. Do not include any markdown, formatting, or line breaks within the strings.`,
            ...
            if (jsonMatch) {
              ...
              if (isMounted.current) {
                setExplanations({
                  ...
                  context:
                    sections[2] ||
                    "This code functions as part of the larger component/class structure.",
                  why:
                    sections[3] ||
                    "This code is necessary to ensure proper functionality and user experience.",
                });
              }
            }
      ...
      const errorMsg =
        err instanceof Error ? err.message : "An error occurred while generating explanation";
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <div
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <label style={{ fontSize: "13px", color: "#666", fontWeight: "500" }}>Mode:</label>
            <select
...
const OutputVisualizer: React.FC<AnnotationEditorProps> = (props) => {
  ...
  const [llmExplanation, setLlmExplanation] = useState(props.value.metadata.llmExplanation || "");
  const [confidence, setConfidence] = useState(props.value.metadata.confidence || "medium");
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [testInputs, setTestInputs] = useState(props.value.metadata.testInputs || "");
  const [actualOutput, setActualOutput] = useState(props.value.metadata.actualOutput || null);
  const [executionError, setExecutionError] = useState(props.value.metadata.executionError || null);
  ...
  const analyzeCode = async () => {
    ...
    setTimeout(() => {
      ...
      if (code.includes("quickSort") && code.includes("partition")) {
        ...
        setLlmExplanation(
          "The highlighted 'quickSort' function sorts the input array 'arr' in-place using the partitioning logic defined in the 'partition' function. The 'arr' variable is modified directly during the sorting process."
        );
        setConfidence("high");
      }
      // Check if it's the partition function
        ...
        setPredictedOutput(
          "The index position where the pivot element is placed after partitioning"
        );
        setVariableChanges([
          {
            variable: "arr",
        ...
        setLlmExplanation(
          "The 'partition' function rearranges the array segment between 'low' and 'high' indices, using the last element as a pivot. It returns the final position of the pivot after partitioning."
        );
        setConfidence("high");
      }
      // Generic function detection
        ...
        setVariableChanges([]);
        setLlmExplanation(
          "This appears to be a function definition. The actual output will depend on the input parameters provided when the function is called."
        );
        setConfidence("low");
      }
      // Default case
        ...
        setVariableChanges([]);
        setLlmExplanation(
          "The highlighted code performs operations. Specific output depends on the context and any variables it uses."
        );
        setConfidence("low");
      }
...
const FormalSpecification: React.FC<AnnotationEditorProps> = (props) => {
  ...
  const renderSplitBox = (title: string, spec: { formal: string; simple: string }) => {
      ...
      <details
        ...
        <div style={{ display: "flex", minHeight: "180px" }}>
          <div
            ...
            <hr />
            {spec.formal || <span style={{ color: "#999" }}>No specification yet</span>}
          </div>
          <div
            style={{
              flex: 1,
            ...
            <hr />
            {spec.simple || <span style={{ color: "#999" }}>No explanation yet</span>}
          </div>
        </div>
      </details>
    );
...
const Generator: React.FC<AnnotationEditorProps> = (props) => {
  ...
  const [GeneratedComponent, setGeneratedComponent] =
    useState<React.FC<AnnotationEditorProps> | null>(null);
...
return React.createElement('div',
  ...
  useEffect(() => {
    ...
    if (lmApi) {
      diagnostics.push(
        `lmApi.chat: ${typeof lmApi.chat} ${typeof lmApi.chat === "function" ? "✓" : "✗"}`
      );
    }

    diagnostics.push(
      `fallbackMockLmApi: ${typeof fallbackMockLmApi} ${fallbackMockLmApi ? "✓" : "✗"}`
    );
    ...
    const lmApiAvailable =
      typeof lmApi !== "undefined" && lmApi !== null && typeof lmApi.chat === "function";
...
Generate the component implementation now.`,
      ...
      if (useMock) {
        ...
        response = await fallbackMockLmApi.chat(
          prompt,
          undefined,
          "Manual fallback mode enabled - real API was not attempted"
        );
        ...
        try {
          ...
          if (apiError instanceof Error) {
            ...
            errorDetails = `Error: ${String(apiError)}\n\nType: ${
              (apiError as any)?.constructor?.name || "unknown"
            }`;
          }
          ...
          if (typeof apiError === "object" && apiError !== null) {
            ...
            const extraProps = Object.keys(errorObj).filter(
              (k) => k !== "message" && k !== "stack" && k !== "name"
            );
      ...
      const errorMsg =
        err instanceof Error ? err.message : "An error occurred while generating the annotation";
  ...
  const compileComponent = (code: string) => {
    try {
      ...
      const WrappedComponent: React.FC<AnnotationEditorProps> = (componentProps) => {
        return componentFunction(React, componentProps, lmApi, mockLmApi, vscode, commonTextStyle);
      };
      ...
      throw new Error(
        `Failed to compile generated component: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  };
...
export { Generator, FormalSpecification, ExplainInEnglish, OutputVisualizer };
