...
const HTMLEditor = (props: {
  ...
  const numOverlappingAnnotations = (index: number) => {
    return annotations.filter((annotation) => index >= annotation.start && index <= annotation.end)
      .length;
  };

  const maxOverlappingAnnotations = [...new Array(documentContent.length)].reduce(
    (acc, _, index) => {
  ...
  const getOpacity = (index: number) => {
    ...
    return numOverlapping === 0
      ? 0
      : minOpacity + (maxOpacity - minOpacity) * (numOverlapping / maxOverlappingAnnotations);
  };
...
function App() {
  // File path: vscode.window.activeTextEditor?.document.uri.fsPath;
  const [documentURI, setDocumentURI] = useState(localStorage.getItem("DocumentURI") || "");
  const [stateURI, setStateURI] = useState(
    localStorage.getItem("StateURI") || getStateURI(localStorage.getItem("DocumentURI")) || ""
  );
      ...
      <div className="annotator">
        {annotations !== undefined && (
          <HTMLEditor
            ...
            setSelectedAnnotation={setSelectedAnnotation}></HTMLEditor>
        )}
      </div>
      <div className="tools">
        <div className="App">
          ...
          <div className="retag-document">
            ...
            <button
              ...
              style={{
                backgroundColor:
                  documentOutOfDate &&
                  !(continuousRetag || documentURI === "" || stateURI === "" || APIKey === "")
                    ? "chartreuse"
                    : "initial",
              }}>
              Retag
