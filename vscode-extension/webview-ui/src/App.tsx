...
function App() {
  ...
  const [stateURI, setStateURI] = useState(
    getStateURI(vscode.window.activeTextEditor?.document.uri.fsPath) || ""
  );
...
function getStateURI(docURI: string | null | undefined): string {
  if (!docURI) return "";
  const re: RegExp = /^(.*\/)([^/]+)$/;
  const match: RegExpMatchArray | null = docURI.match(re);
  if (match && match.length === 3) {
    return "/annotations/" + "." + match[2] + ".ann.json";
  }
  return "";
}
